...
if TYPE_CHECKING:
    from ai_gateway.chains import BaseChainRegistry
...
def _react_agent_factory(
    chain_registry: "BaseChainRegistry",
    ...
    def _fn(tools: Sequence[BaseTool], inputs: ReActAgentInputs) -> ReActAgent:
        ...
        return chain_registry.get("chat/react", options, inputs.model_metadata)

    return _fn


class ContainerChat(containers.DeclarativeContainer):
    chains = providers.DependenciesContainer()
    ...
    _react_agent_factory = providers.Factory(
        ...
        chain_registry=chains.chain_registry,
    )
