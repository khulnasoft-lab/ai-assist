...
from jose import (  # <--- I guess we can rely on it as dependency? (will list in requirements.txt?)
    JWSError,
    JWTError,
    jwt,
)

__all__ = ["TokenAuthority", "RS256_ALGORITHM", "SELF_SIGNED_TOKEN_ISSUER"]
...
SELF_SIGNED_TOKEN_ISSUER = (
    "gitlab-ai-gateway"  # This should passed into constructor of TokenAuthority!
)
...
class TokenAuthority:
    ...
    def encode(self, sub, gitlab_realm) -> ServiceResponse:
        ...
        try:
            ...
            JWTError,
            JWSError,
        ) as err:  # Interesting, that it is JW(S)Eror, not JW(T). Did we have a bug here?
