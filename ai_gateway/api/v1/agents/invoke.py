...
async def agent(
    ...
    try:
        agent = agent_registry.get_on_behalf(current_user, agent_request.id)
    except KeyError:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=f"Agent '{agent_request.id}' not found",
        )
    except WrongUnitPrimitives:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail=f"Unauthorized to access '{agent_request.id}'",
        )
    ...
    if not type(agent) is Agent:  # pylint: disable=unidiomatic-typecheck
        raise HTTPException(
            ...
            detail=f"Agent '{agent_request.id}' is not supported",
        )
